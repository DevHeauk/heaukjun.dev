...
| readConcern | What you see |
|---|---|
| `local` | The newest value on this node. Can still be rolled back |
| `majority` | A value acknowledged by a majority, so it won't be rolled back |
| `snapshot` | A consistent snapshot from when the transaction began (inside a transaction) |
