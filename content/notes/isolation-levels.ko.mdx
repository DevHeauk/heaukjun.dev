...
MongoDB에는 `READ COMMITTED` 같은 격리 수준 설정이 없습니다. 대신 **readConcern**과 **writeConcern**으로 나눠 표현합니다.
...
관계형과 대응시키면 대략 이렇습니다. `local`이 READ UNCOMMITTED에 가깝고(롤백 가능성), `majority`가 READ COMMITTED, 트랜잭션 안의 `snapshot`이 REPEATABLE READ 역할을 합니다.
